...
class Google_sync
{
    ...
    public function add_appointment(
        ...
        // START - Jens Willemsens - 2026-02-24: Add custom summary
        $summary = (!empty($service) ? $service['name'] : 'Onbekende dienst') . ' - ' . (!empty($customer) ? $customer['first_name'] : 'Onbekende klant');
        if (!empty($customer) && !empty($customer['last_name'])) {
            $summary .= ' ' . $customer['last_name'];
        }
        $event->setSummary($summary);
        // END - Jens Willemsens - 2026-02-24: Add custom summary
        // START - Jens Willemsens - 2026-02-24: Add custom description
        $description = '';
        if (!empty($appointment) && !empty($appointment['notes'])) {
            $description = "Afspraak notities:\n" . $appointment['notes'];
        }
        if (!empty($customer) && !empty($customer['notes'])) {
            if (!empty($description)) {
                $description .= "\n\n";
            }
            $description .= "Klant notities:\n" . $customer['notes'];
        }
        $event->setDescription($description);
        // END - Jens Willemsens - 2026-02-24: Add custom description
        ...
        // START - Jens Willemsens - 2026-02-24: Disable event attendees
        // $event->attendees = [];
        ...
        // }
        // END - Jens Willemsens - 2026-02-24: Disable event attendees
